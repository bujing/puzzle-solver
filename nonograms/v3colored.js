      ...
      if (this.#backtracking) {
        ...
        while (last > -1) {
          ...
          if (value === 'x') {
            ...
            this.#tryAndError[last][3] = this.#tryAndError[last][4] === colors.length ? 'x' : colors[this.#tryAndError[last][4]]
        ...
        for (let row = 0; row < this.#rows; row++) {
          ...
          if (column > -1) {
            const colors = this.colors[row][column]
            this.#tryAndError.push([this.#grid.length, row, column, colors[0], 1])
            const grid = this.clone(this.latest)
      ...
      for (let row = 0; row < this.#rows; row++) {
        ...
        for (let column = 0; column < this.#columns; column++) {
          if (grid[row][column] === 'u') {
            ...
            if (colors.length === 0) { // 没有 color 校验通过
              grid[row][column] = 'x'
              loop = true
            } else if (colors.length === 1) { // 只有一个 color 校验通过
              // 尝试 x
              grid[row][column] = 'x'
